      ...
      switch(e.type) {
        case SDL_MOUSEBUTTONUP:
          if(e.button.button == SDL_BUTTON_RIGHT) {
            AddPoint(AllocPointLink(-1 + 2. * e.button.x / WINDOW_WIDTH,
              -(-1 + 2. * e.button.y / WINDOW_HEIGHT), 255, 200, 100), &primitives->points);
        ...
        case SDL_KEYUP:
          if(e.key.keysym.sym == SDLK_l) {
            /* Si l relachée, on ajoute un nouveau maillon dans la liste de primitive.
            Les futur points cliqué seront ajouté dans celui ci. */
